...
Run the install script to set up the MCP server with Claude Desktop:

```bash
curl -sL https://raw.githubusercontent.com/YOUR_REPO/meta-ads-mcp/main/install.sh | bash
```

Or manually:

1. Install: `pip install -e .`
2. Set `META_ACCESS_TOKEN` environment variable with your Meta access token
3. Run: `python -m meta_ads_mcp`
...
Meta Ads MCP follows security best practices with secure token management. Tokens are stored locally on your machine.
...
1. Verify your `META_ACCESS_TOKEN` is valid and not expired
2. Check that the token has the required permissions (ads_management, ads_read)
3. Generate a new token at https://developers.facebook.com/tools/explorer/
