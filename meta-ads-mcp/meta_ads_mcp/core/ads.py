...
from .ryzon_defaults import apply_ryzon_ad_defaults, apply_ryzon_creative_defaults
...
async def create_ad(
    ...
    """
    # Apply Ryzon defaults if RYZON_MODE is active
    account_id, status, tracking_specs = apply_ryzon_ad_defaults(
        account_id, status, tracking_specs
    )
        ...
        if file:
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "suggestions": [
                        "Try uploading the image via the upload_ad_image tool with a publicly accessible URL, then use the image hash directly.",
...
async def create_ad_creative(
    ...
    # Apply Ryzon defaults if RYZON_MODE is active
    account_id, page_id, instagram_actor_id, creative_features_spec, url_tags = (
        ...
        if instagram_actor_id and "error" in data:
            ...
            if "valid Instagram account id" in inner_msg or "instagram_actor_id" in inner_msg.lower():
                return json.dumps({
                    ...
                    "fix": (
                        "Regenerate your access token with the 'instagram_basic' permission "
